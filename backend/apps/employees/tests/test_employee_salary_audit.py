...
class TestEmployeeSalaryUpdate:
	...
	def test_non_salary_update_does_not_create_audit(self, employee):
		client = APIClient()
		client.patch(reverse("employee-detail", args=[employee.id]), {"job_title": "Senior Engineer"}, format="json")
	...
	def test_multiple_salary_changes_create_multiple_audits(self, employee):
		client = APIClient()
		client.patch(reverse("employee-detail", args=[employee.id]), {"salary": "60000.00"}, format="json")
		client.patch(reverse("employee-detail", args=[employee.id]), {"salary": "70000.00"}, format="json")
