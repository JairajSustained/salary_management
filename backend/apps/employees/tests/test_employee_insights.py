...
class TestEmployeeInsights:
    ...
    def test_insights_response_has_by_department_key(
        self, api_client, insights_employees
    ):
        response = api_client.get(reverse("employee-insights"))
    ...
    def test_insights_by_country_entry_has_required_fields(
        self, api_client, insights_employees
    ):
        response = api_client.get(reverse("employee-insights"))
    ...
    def test_insights_by_department_entry_has_required_fields(
        self, api_client, insights_employees
    ):
        response = api_client.get(reverse("employee-insights"))
        entry = next(
            e
            for e in response.data["by_department"]
    ...
    def test_insights_by_title_entry_has_required_fields(
        self, api_client, insights_employees
    ):
        response = api_client.get(reverse("employee-insights"))
        entry = next(
            e
            for e in response.data["by_title"]
            if e["job_title"] == "Software Engineer"
        )
        assert "min_salary" in entry
        assert "max_salary" in entry
        assert "avg_salary" in entry
    ...
    def test_insights_by_country_salary_calculations(
        self, api_client, insights_employees
    ):
    ...
    def test_insights_by_department_salary_calculations(
        self, api_client, insights_employees
    ):
    ...
    def test_insights_by_title_salary_calculations(
        self, api_client, insights_employees
    ):
