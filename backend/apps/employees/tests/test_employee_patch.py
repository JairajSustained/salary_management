...
class TestEmployeePatch:
	...
	def test_employee_patch_returns_200(self, api_client, employee):
		response = api_client.patch(reverse("employee-detail", args=[employee.id]), {"salary": "60000.00"}, format="json")
		assert response.status_code == 200

	def test_employee_patch_updates_field(self, api_client, employee):
		api_client.patch(reverse("employee-detail", args=[employee.id]), {"salary": "60000.00"}, format="json")
	...
	def test_employee_patch_nonexistent_returns_404(self, api_client):
		response = api_client.patch(reverse("employee-detail", args=[uuid.uuid4()]), {"salary": "60000.00"}, format="json")
		assert response.status_code == 404

	def test_employee_patch_returns_updated_data(self, api_client, employee):
		response = api_client.patch(reverse("employee-detail", args=[employee.id]), {"job_title": "Senior Engineer"}, format="json")
